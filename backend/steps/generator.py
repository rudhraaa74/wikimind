...
9. STRICT CITATION FORMATTING: DO NOT cite every sentence. This is a critical formatting rule. If an entire paragraph draws from the same source, you MUST place only ONE citation at the very end of that paragraph. Multiple consecutive sentences from the same source should NEVER have individual citations. Group them into a cohesive paragraph and cite once at the end.
...
11. FORMATTING: You must enclose all mathematical formulas, variables, and scientific notation (like solar masses, equations, powers of 10) in `$` for inline math (e.g. `$M_\odot$`) and `$$` for block equations. Never use raw unicode math symbols outside of these math block delimiters.
"""
